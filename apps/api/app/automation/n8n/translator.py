...
Mapping (see each _translate_* function's docstring for why):
  ...
  action   email.send                -> n8n-nodes-base.emailSend     (credential by reference)
...
def _translate_action_node(
    ...
    if node.action is ActionType.EMAIL_SEND:
        ...
        return _email_send_node(node.id, x_position, node.inputs, context.email_credential_id)
...
def _email_send_node(node_id: str, x_position: int, inputs: EmailSendInputs, credential_id: str) -> dict[str, Any]:
    """`to` is a role (customer/business_owner), resolved at execution
    time from the triggering payload/business, not a literal address
    baked in here. `credentials` references an SMTP credential n8n
    already has *by id* — the raw SMTP password never passes through
    this translator or the generated JSON, satisfying "credentials
    referenced by identifier, not value"."""
    return {
        "id": node_id,
        "name": node_id,
        ...
        "parameters": {
            ...
            "toEmail": f"={{{{ $json.{inputs.to.value} }}}}",
            "subject": inputs.subject or "",
            "emailFormat": "text",
            "text": f"={{{{ $json.templates.{inputs.template} }}}}",
        },
